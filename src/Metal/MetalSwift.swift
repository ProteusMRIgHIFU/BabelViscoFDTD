...
{
    #if DRYTEST
        print("ConstantBuffers: lenconstuint, lenconstmex ",lenconstuint, lenconstmex)
    #else
...
public func SymbolInitiation_uint(index: UInt32, data: UInt32) -> Int {
    #if DRYTEST
        print("SymbolInitiation_uint: index, data ", index, data)
    #else
...
public func SymbolInitiation_mex(index: UInt32, data:Float32) -> Int{
    #if DRYTEST
        print("SymbolInitiation_mex: index, data ", index, data)
    #else
...
public func BufferIndexCreator(c_mex_type:UnsafeMutablePointer<UInt64>, c_uint_type:UInt64, length_index_mex:UInt64, length_index_uint:UInt64) -> Int {
    #if DRYTEST
        print("BufferIndexCreator: c_uint_type, length_index_mex ", c_uint_type, length_index_mex,length_index_uint)
    #else
...
public func IndexManipMEX(data:UInt32, data2:UInt32, index:UInt32) -> Int{
    #if DRYTEST
        print("IndexManipMEX: data, data2, index ", data, data2, index)
    #else
...
public func IndexManipUInt(data:UInt32, data2:UInt32, index:UInt32) -> Int{
    #if DRYTEST
        print("IndexManipUInt: data, data2, index ", data, data2, index)
    #else
        var ll = Int(index) * 2 * MemoryLayout<UInt32>.stride
...
public func IndexDidModify(lenind_mex:UInt64, lenind_uint:UInt64, lenconstmex:UInt64, lenconstuint:UInt64){
    #if DRYTEST
        print("IndexDidModify: lenind_uint, lenconstmex, lenconstuint ", lenind_mex, lenconstmex, lenconstuint)
    #else
...
{   
    #if DRYTEST
        print("CompleteCopyMEX: size, ind, buff", size, ind, buff)
    #else
...
{
    #if DRYTEST
        print("CompleteCopyMEX: size, ind, buff ", size, ind)
    #else
...
public func GetFloatEntries(c_mex_type: UnsafeMutablePointer<UInt64>, c_uint_type: UInt64) -> UInt64
{
        ...
        print("GetFloatEntries: c_uint_type, floatCounter", c_uint_type,floatCounter)
...
public func GetMaxTotalThreadsPerThreadgroup(fun:UnsafeRawPointer, id:Int) -> UInt32{
    ...
    if id == 0{
        #if DRYTEST
                print("GetMaxTotalThreadsPerThreadgroup func_name, id, index, maxTotalThreadsPerThreadgroup", func_name, id, index,stress_funcs[index]!.maxTotalThreadsPerThreadgroup)
        #endif
        return UInt32(stress_funcs[index]!.maxTotalThreadsPerThreadgroup)
    }
    else{
        #if DRYTEST
                print("GetMaxTotalThreadsPerThreadgroup func_name, id, index, maxTotalThreadsPerThreadgroup", func_name, id, index,particle_funcs[index]!.maxTotalThreadsPerThreadgroup)
        #endif
...
public func GetThreadExecutionWidth(fun:UnsafeMutablePointer<CChar>, id:Int)-> UInt32 {
    ...
    if id == 0{
        #if DRYTEST
                print("GetThreadExecutionWidth func_name, id, index, maxTotalThreadsPerThreadgroup", func_name, id, index,stress_funcs[index]!.threadExecutionWidth)
        #endif
        return UInt32(stress_funcs[index]!.threadExecutionWidth)
    }
    else{
        #if DRYTEST
                print("GetThreadExecutionWidth func_name, id, index, maxTotalThreadsPerThreadgroup", func_name, id, index,particle_funcs[index]!.threadExecutionWidth)
        #endif
...
public func EncodeStress(fun:UnsafeRawPointer, i:UInt32, j:UInt32, 
    ...
    let func_name = NSString(bytes:fun, length: 5, encoding:String.Encoding.utf8.rawValue)
    #if DRYTEST
        print("EncodeStress func_name,i,j,k,x,y,z,Gx,Gy,Gz  ",String(func_name!),i,j,k,x,y,z,Gx,Gy,Gz)
    #else
        var ind:Int!
        for name in func_names{
            if name.contains(func_name as! String){
...
public func EncodeParticle(fun:UnsafeRawPointer, i:UInt32, j:UInt32, k:UInt32, x:UInt32, y:UInt32, z:UInt32,
    ...
    let func_name = NSString(bytes:fun, length: 5, encoding:String.Encoding.utf8.rawValue)
    #if DRYTEST
        print("EncodeParticle func_name,i,j,k,x,y,z,Gx,Gy,Gz ",String(func_name!),i,j,k,x,y,z,Gx,Gy,Gz)
    #else
        var index:Int!
        for name in func_names{
            if name.contains(func_name as! String){
...
public func EncodeSensors(i:UInt32, j:UInt32, k:UInt32, x:UInt32, y:UInt32, z:UInt32){
    #if DRYTEST
        print("EncodeSensors i,j,k,x,y,z ",i,j,k,x,y,z)
    #else
...
public func maxThreadSensor() -> Int{
    #if DRYTEST
        print("maxThreadSensor " , computePipelineState_Sensors.maxTotalThreadsPerThreadgroup)
    #endif
